...
cy.factory('socketService', ['$rootScope', function ($rootScope) {
...
cy.controller('cyController', ['$rootScope', 'socketService', function($rootScope, socketService) {
	...
	socketService.on('updated', function (data) {
		for (var i = 0; i < data.length; i ++) {
			$rootScope.resMenus[i].num = data[i].num;
		}
		$rootScope.count();
	});
	...
	$rootScope.add = function($event) {
		...
		this.count();
		$rootScope.noitemsDisplay = false;
		socketService.emit('update', $rootScope.resMenus);
	};
